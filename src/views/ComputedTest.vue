<template>
  <div class="container d-flex flex-column my-3 px-0 py-3">
    ...
    <section class="mb-3 p-3">
      <h2 class="text-20 mb-1" :data-theme="theme">1. computed 排序data</h2>
      ...
      <p class="my-2" :data-theme="theme"><code>{{ sortData }}</code></p>
...
const sortData = computed(() => data.value.sort((a, b) => isSortReverse.value ? a - b : b - a))</pre>
    ...
    <!-- Computed get(), set() -->
    <section class="mb-3 p-3">
      <h2 class="text-20 mb-1" :data-theme="theme">
...
function getRandomDiscount () {
  discountPrice.value = Math.random().toFixed(2)
    ...
    <section class="mb-3 p-3">
      ...
      <label for="emailInput" class="form-label my-2" :data-theme="theme">
        請輸入信箱
      ...
      <p class="mt-1 mb-2" :data-theme="theme">
        信箱: "<code>{{ email }}</code>"
      </p>
