<template>
  <div class="container d-flex flex-column my-3 px-0 py-3">
    <section class="my-3 px-3">
      <h2 class="text-20 mb-1" :data-theme="themeStore.theme">Slot 插槽</h2>
    </section>

    <!-- 預設插槽 (默認插槽) -->
    <section class="px-3">
      <h3 class="text-18 mb-3" :data-theme="themeStore.theme">1. 預設插槽</h3>
      <!-- 父 -->
      <div class="ms-3">
        <h4 class="mb-1" :data-theme="themeStore.theme">父層</h4>
  ...
  <span>預設插槽的文字</span>
      ...
      <!-- 子 -->
      <SlotChild1>
        <span :data-theme="themeStore.theme">預設插槽的文字</span>
      </SlotChild1>
    </section>

    <!-- 具名插槽 -->
    <section class="px-3">
      <h3 class="text-18 mb-3" :data-theme="themeStore.theme">2. 具名插槽</h3>
      <!-- 父 -->
      <div class="ms-3">
        <h4 class="mb-1" :data-theme="themeStore.theme">父層</h4>
  ...
  <template #text>
    <span>具名插槽</span>
      ...
      <SlotChild2>
        <template #text>
          <span :data-theme="themeStore.theme">具名插槽</span>
        </template>
      </SlotChild2>
    </section>
    ...
    <!-- 作用域插槽 -->
    <section class="px-3">
      <h3 class="text-18 mb-3" :data-theme="themeStore.theme">3. 作用域插槽 <span class="text-14"> ( 把子層的資料取出來給父層用 )</span></h3>
      <!-- 父 -->
      <div class="ms-3 mb-3">
        <h4 class="mb-1" :data-theme="themeStore.theme">父層</h4>
      ...
      <SlotChild3>
        <template #text2="slotProps">
          <p :data-theme="themeStore.theme">1. 普通寫法：{{ slotProps }}"</p>
          <pre class="bg-dark text-light mt-1 ms-3 mb-4 p-2">{{ slotExample1 }}</pre>
        </template>
        <!-- 解構 -->
        <template #text3="{text3}">
          <p :data-theme="themeStore.theme">2. 解構寫法：{{ text3 }}"</p>
    ...
    <!-- 動態插槽 -->
    <section class="px-3">
      <h3 class="text-18 mb-3" :data-theme="themeStore.theme">4. 動態插槽</h3>
      <!-- 父 -->
      <div class="ms-3">
        <h4 class="mb-1" :data-theme="themeStore.theme">父層</h4>
        ...
        <label for="插槽1" class="me-3" :data-theme="themeStore.theme" >
          <input type="radio" name="dynamic" value="插槽1" id="插槽1"  v-model="dynamicSlot">
          插槽1
        </label>
        <label for="插槽2" class="me-3" :data-theme="themeStore.theme" >
          <input type="radio" name="dynamic" value="插槽2" id="插槽2"  v-model="dynamicSlot">
          插槽2
        </label>
        <label for="插槽3" class="me-3" :data-theme="themeStore.theme" >
          <input type="radio" name="dynamic" value="插槽3" id="插槽3"  v-model="dynamicSlot">
          插槽3
        </label>
        <p :data-theme="themeStore.theme">當前插槽：<code>{{ dynamicSlot }}</code></p>
      </div>

      <!-- 子 -->
      <SlotChild4>
        <template v-slot:[dynamicSlot]>
          <span :data-theme="themeStore.theme">動態插槽的變數：<code>{{ dynamicSlot }}</code></span>
        </template>
      </SlotChild4>
    </section>
  </div>
